...
{
    protected function createExamCont($subject_name, $exam_name,$exam_date, $exam_start,$duration, $total_mark  )
    {
        $sql = "INSERT into exams(subject_id, exam_name, exam_date, start_time, duration, total_mark) values ( ?, ?,?,?,?,?)";
        $stmt = $this->Connection()->prepare($sql);
        $stmt->execute(array($subject_name, $exam_name,$exam_date, $exam_start,$duration, $total_mark));
    }
